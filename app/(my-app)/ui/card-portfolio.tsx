...
export default function CardPortfolio({
  ...
  function isProject(
    item: Project | NonNullable<ImagesBlock["images"]>[number],
  ): item is Project {
    return "slug" in item;
  }
  ...
  const projectImage =
    isProject(item) && typeof item.thumbnail === "object"
      ? item.thumbnail
      : null;
  const imageBlock =
    !isProject(item) && typeof item.image === "object" ? item.image : null;

  return (
      <div
        className={clsx(
          "w-full overflow-hidden rounded-lg shadow-lg cursor-pointer group",
          {
            "h-auto relative aspect-[4/2.65] bg-purple-300": isProject(item),
          },
          {
            "max-h-[700px]":
              !isProject(item),
