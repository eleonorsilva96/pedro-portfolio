...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      ...(category.meta?.description && { description: category.meta.description }), 
      images: typeof category.meta?.image === 'object' && category.meta?.image?.url ? [{ url: category.meta.image.url }] : [],
    },
  }
}
...
export default async function PortfolioPage({
  ...
  const getVisibleProjects = projects.filter((project) => project.hideProject === false);
  ...
  const mainContent = !isMusicCategory ? (
    ...
    <>
      ...
      <div className="flex justify-center w-full mt-8 py-14 bg-neutral-400/50 ">
        ...
        {musicCTA && <RichText data={musicCTA} className="rich-text text-4xl" />}
      </div>
    </>
  );

  const modalGallery = isModal && id ? projectsData : null;
  const modalContent = isModal && id ? projectsData.find((project) => project.id === id) : null;
    ...
    <div className="flex flex-col w-full items-center">
      ...
      {category?.description ? (
        ...
        <div className="text-center w-full md:w-2xl lg:w-3xl text-lg px-4 min-w-0 break-words"> 
          {/* use Payload rich text renderer to render the lexical rich text object */}
            {category?.description && <RichText data={category.description} className="rich-text"/>}
        </div>
      ) : (
        ""
      )}
